...
/**
 *Submitted for verification at basescan.org on 2023-10-16
*/
...
// File @openzeppelin/contracts-0.8/utils/Address.sol@v4.4.2
...
// File @openzeppelin/contracts-0.8/token/ERC20/utils/SafeERC20.sol@v4.4.2
...
contract RewardPoolDepositWrapper {
    ...
    function depositMutipleAndLock(
        ...
        // 1. Transfer inputs token
        _inputToken1.safeTransferFrom(msg.sender, address(this), _inputAmount1);

        _inputToken2.safeTransferFrom(msg.sender, address(this), _inputAmount2);

        // 2. Deposit to balancer pool
        (address pool, ) = bVault.getPool(_balancerPoolId);

        _inputToken1.approve(address(bVault), _inputAmount1);
        _inputToken2.approve(address(bVault), _inputAmount2);
        ...
        uint256 inputBalAfter1 = _inputToken1.balanceOf(address(this));
        if (inputBalAfter1 != 0) {
            // Refund any amount left in the contract
            _inputToken1.transfer(msg.sender, inputBalAfter1);
        }

        uint256 inputBalAfter2 = _inputToken2.balanceOf(address(this));
        if (inputBalAfter2 != 0) {
            // Refund any amount left in the contract
            _inputToken2.transfer(msg.sender, inputBalAfter2);
        }
